...
const html = {
    ...
    screenShare: 'fas fa-desktop',
    stopRecording: 'fas fa-record-vinyl',
    stopScreen: 'fas fa-stop-circle',
...
const image = {
    about: '../images/pedagobox-logo.gif',
    avatar: '../images/user.png',
    audio: '../images/audio.gif',
    poster: '../images/maintenance.gif',
...
class RoomClient {
    ...
    initSockets() {
        ...
        //     'unmute',
        //     function (data) {
        //         userLog('info', 'Manager Enabled your audio.');
        ...
        this.socket.on(
            ...
            function (data) {
                Swal.fire({
                    ...
                    icon: 'warning',
                    title: 'Alert',
                    text: 'Manager enabled your screen share functoin. \n Please share your screen now',
                    timer: 3000,
                    ...
                    document.getElementById('startScreenButton').classList.remove('hiddenIcon');
                    document.getElementById('startScreenButton').click();
                    this.socket.emit('shareSuccess', { peer_id: data.peer_id, manager_id: data.manager_id });
                });
            }.bind(this),
        );

        this.socket.on('Register_manager', function (data) {
            console.log('register room_manager', data);
            window.localStorage.room_manager = data;
        });

        this.socket.on('shareSuccessed', function (data) {
            // alert('successfully share enabled for ' + data);
            let share_id = 'remotePeer__' + data + '__screenShare';
            let stop_id = 'remotePeer__' + data + '__stopScreenShare';
            document.getElementById(share_id).classList.add('hiddenIcon');
            document.getElementById(stop_id).classList.remove('hiddenIcon');
        });

        this.socket.on('stopShareScreen', (data) => {
            document.getElementById('stopScreenButton').click();
            document.getElementById('startScreenButton').classList.add('hiddenIcon');

            this.socket.emit('stopSuccess', data);
        });

        this.socket.on('disableSuccess', function (data) {
            let share_id = 'remotePeer__' + data + '__screenShare';
            let stop_id = 'remotePeer__' + data + '__stopScreenShare';
            document.getElementById(share_id).classList.remove('hiddenIcon');
            document.getElementById(stop_id).classList.add('hiddenIcon');
        });

        this.socket.on('record-start', function (data) {
            let record_id = 'remotePeer__' + data.peer_id + '__stopRecording';
            document.getElementById(record_id).classList.remove('hiddenIcon');
            document.getElementById('stopRecordButton').classList.remove('hidden');
        });

        this.socket.on('stop-all-rec', (data) => {
            document.getElementById('stopRecButton').click();
            this.socket.emit('stop-rec-success', peer_info);
        });

        this.socket.on('stop-rec-success', (data) => {
            document.getElementById('remotePeer__' + data.peer_id + '__stopRecording').classList.add('hiddenIcon');
            stopFlag++;
            this.socket
                ...
                .then(
                    async function (data) {
                        ...
                        if (data.peerCounts - 1 == stopFlag) {
                            // alert('all participants\' recording stopped!');
                            document.getElementById('stopRecordButton').classList.add('hidden');
                        }
                    }.bind(this),
                )
                .catch((err) => {
        ...
        this.socket.on('receiveRecording', function (recordedData) {
            // alert(recordedData);
            userLog('info', 'Recording data from ' + recordedData + ' saved successfully!', 'top-end');
            // alert(recordedData);
            // const newDate = new Date();
            // const date = newDate.toISOString().split('T')[0];
            // const time = newDate.toTimeString().split(' ')[0];

            // // const type = recordedData[0].type.includes('mp4') ? 'mp4' : 'webm';
            // const blob = new Blob(recordedData.recordedData, { type: 'video/' + 'webm' });
            // const recFileName = recordedData.peer + '_(' + window.localStorage.room_name + ')_' + `${date}-${time}` + '-REC.' + 'webm';
    ...
    handleConsumer(id, type, stream, peer_name, peer_info) {
        let elem, vb, d, p, i, cm, au, pip, fs, ts, sf, sm, sv, ko, pb, pm, pv, pn, sr, st, ss;
        ...
        if (
            remotePeerId != window.localStorage.room_manager &&
            remotePeerId != window.localStorage.peer_id &&
        ...
        switch (type) {
            ...
            case mediaType.screen:
                ...
                sr.id = id + '__' + remotePeerId + '__stopRecording';
                sr.className = html.stopRecording;
                sr.classList.add('color-green');
                ss = document.createElement('button');
                ss.id = id + '__' + remotePeerId + '__screenShare';
                ss.className = html.screenShare;
                st = document.createElement('button');
                st.id = id + '__' + remotePeerId + '__stopScreenShare';
                st.className = html.stopScreen;
                st.classList.add('color-red');
                if (window.localStorage.role != 1) st.classList.add('hiddenIcon');
                ...
                BUTTONS.consumerVideo.stopScreenShareButton && vb.appendChild(st);
                ...
                this.handleST(st.id, peer_name);
                this.handleSS(ss.id, peer_name);
                this.handleSV(sv.id);
                ...
                if (!this.isMobileDevice) {
                    ...
                    this.setTippy(st.id, 'Disable Screen Share', 'top-end');
                    this.setTippy(sr.id, 'Record started!', 'top-end');
                    this.setTippy(ss.id, 'Screen Share', 'top-end');
    ...
    async setVideoOff(peer_info, remotePeer = false) {
        let d, vb, i, h, au, sf, sm, sv, ko, p, pm, pb, pv, sr, ss, st;
        let peer_id = peer_info.peer_id;
        let peer_name = peer_info.peer_name;
        let peer_audio = peer_info.peer_audio;
        if (remotePeer && peer_id != window.localStorage.room_manager && window.localStorage.role != 1) return;
        ...
        if (remotePeer) {
            ...
            sm.className = html.sendMsg;
            sr = document.createElement('button');
            sr.id = 'remotePeer__' + peer_id + '__stopRecording';
            sr.className = html.stopRecording;
            sr.classList.add('color-green');
            sr.classList.add('hiddenIcon');
            ss = document.createElement('button');
            ss.id = 'remotePeer__' + peer_id + '__screenShare';
            ss.className = html.screenShare;
            st = document.createElement('button');
            st.id = 'remotePeer__' + peer_id + '__stopScreenShare';
            st.className = html.stopScreen;
            st.classList.add('hiddenIcon');
            sv = document.createElement('button');
            ...
            ko.className = html.kickOut;
            console.log(window.localStorage.room_manager, peer_id);
            if (window.localStorage.role == 2 && window.localStorage.room_manager != peer_id) {
                pv.classList.add('hiddenIcon');
                sf.classList.add('hiddenIcon');
                sm.classList.add('hiddenIcon');
                st.classList.add('hiddenIcon');
                sr.classList.add('hiddenIcon');
                ss.classList.add('hiddenIcon');
                sv.classList.add('hiddenIcon');
                ko.classList.add('hiddenIcon');
                document.getElementById('chatButton').classList.add('hiddenIcon');
                document.getElementById('startScreenButton').classList.add('hiddenIcon');
                document.getElementById('participantsButton').classList.add('hiddenIcon');
            }
            if (window.localStorage.room_manager == peer_id) {
                ss.classList.add('hiddenIcon');
                document.getElementById('chatButton').classList.add('hiddenIcon');
                document.getElementById('startScreenButton').classList.add('hiddenIcon');
                document.getElementById('participantsButton').classList.add('hiddenIcon');
        ...
        if (remotePeer) {
            ...
            BUTTONS.videoOff.screenShareButton && vb.appendChild(ss);
            BUTTONS.videoOff.stopScreenShareButton && vb.appendChild(st);
            BUTTONS.videoOff.stopRecordingButton && vb.appendChild(sr);
        ...
        if (!this.isMobileDevice && remotePeer) {
            ...
            this.setTippy(sf.id, 'Send file', 'top-end');
            this.setTippy(ss.id, 'Screen Share', 'top-end');
            this.setTippy(st.id, 'Disable share', 'top-end');
            this.setTippy(sr.id, 'Recording Started', 'top-end');
            this.setTippy(sv.id, 'Send video', 'top-end');
            this.setTippy(au.id, 'Mute', 'top-end');
            this.setTippy(pv.id, '🔊 Volume', 'top-end');
            this.setTippy(ko.id, 'Eject', 'top-end');
    ...
    shareScreen() {
        if (!this.isMobileDevice && (navigator.getDisplayMedia || navigator.mediaDevices.getDisplayMedia)) {
            ...
            Swal.fire({
                ...
                position: 'center',
                icon: 'question',
                title: 'You need to share your entire screen and record it now!',
                text: 'Please share your  entire screen',
                showDenyButton: true,
    ...
    handleST(uid, name) {
        let btnSt = this.getId(uid);
        if (btnSt) {
            btnSt.addEventListener('click', () => {
                this.socket.emit('disableShare', { peer_data: btnSt.id });
            });
        }
    }

    handleSS(uid, name) {
        let btnSs = this.getId(uid);
        ...
        if (btnSs) {
            btnSs.addEventListener('click', () => {
                Swal.fire({
                    ...
                    title: "Share current participant's screen?",
                    text: 'Once shared, current participant can share his/her screen!',
                    showDenyButton: true,
                    confirmButtonText: `Yes`,
                    denyButtonText: `No`,
                    showClass: {
                    ...
                    .then((result) => {
                        if (result.isConfirmed) {
                            let peer_data = btnSs.id;
                            this.socket.emit('individualScS', { peer_data: peer_data });
                        }
                    })
                    .then(() => {
                        this.peerActionProgress('info', 'In progress, wait...', 2000, 'refresh');
                    });
            });
        }
    }
    ...
    handleMediaRecorder() {
        if (this.mediaRecorder) {
            this.socket.emit('record-start', this.peer_info);
    ...
    handleMediaRecorderStop = (evt) => {
        try {
            ...
            console.log('recorded file data', blob);
            ...
            formData.append('peer_name', peer_info.peer_name);
            ...
            Swal.fire({
                ...
                <div style="text-align: left;">
                    ...
                    <br/>
                    Please wait to be processed, currently uploading your recorded data from ${currentDevice} to server.
                </div>`,
                showClass: { popup: 'animate__animated animate__fadeInDown' },
                hideClass: { popup: 'animate__animated animate__fadeOutUp' },
            });
            ...
            $.ajax({
                ...
                success: function (res) {
                    console.log(res);
                    userLog('info', 'res', 'top-end');
                },
            });
            this.sendFileInformations(blob, peer_info.peer_id, false);
    ...
    selectFileToShare(peer_id, broadcast = false) {
        ...
        Swal.fire({
            allowOutsideClick: false,
            background: swalBackground,
            imageAlt: 'pedagobox-file-sharing',
            imageUrl: image.share,
            position: 'center',
            title: 'Share file',
            input: 'file',
    ...
    sendFileData(peer_id, broadcast) {
        ...
        this.fileReader.addEventListener('load', (e) => {
            ...
            offset += data.fileData[0].byteLength;
            ...
            if (offset === this.fileToSend.size) {
                ...
                // this.socket.emit('file success', peer_info);
            }
    ...
    handleFile(data) {
        ...
        console.log(this.receivedSize);
        receiveProgress.value = parseInt(this.receivedSize);
        ...
        if (isImageURL(this.incomingFileInfo.fileName)) {
            ...
            reader.onload = (e) => {
                Swal.fire({
                    ...
                    imageAlt: 'pedagobox-file-img-download',
                    showDenyButton: true,
    ...
    // handleUnmute = (data) => {
    //     const btnAU = document.getElementById(data.peer_id + "__audio");
    //     btnAU.addEventListener('click', (e) => {
    //         this.socket.emit('unmute', peer_id);
    //     })
    // }
    ...
    peerAction(from_peer_name, id, action, emit = true, broadcast = false, info = true) {
        ...
        if (emit) {
            ...
            switch (action) {
                case 'mute':
                    var peerAudioStatus = this.getId(data.peer_id + '__audio');
                    ...
                    break;
                case 'unmute':
                    // peerAudioStatus = this.getId(data.peer_id + '__audio');
                    if (!peerAudioStatus || peerAudioStatus.className == html.audioOff) {
                        // this.userLog(
                        //     'info',
                        //     'The participant has been muted, and only they have the ability to unmute themselves',
                    ...
                    break;
                case 'hide':
                    var peerVideoOff = this.getId(data.peer_id + '__videoOff');
                    if (peerVideoOff) {
                        // return this.userLog(
                        //     'info',
            ...
            switch (action) {
                ...
                case 'unmute':
                    if (peer_id === this.peer_id || broadcast) {
                        ...
                        this.userLog(
                            'warning',
                            from_peer_name + '  ' + _PEER.audioOn + ' has enabled yours audio',
                            'top-end',
                            10000,
                        );
                    }
                ...
                case 'unhide':
                    if (peer_id === this.peer_id || broadcast) {
                        ...
                        this.userLog(
                            'warning',
                            from_peer_name + '  ' + _PEER.videoOn + ' has opened yours video',
                            'top-end',
                            10000,
                        );
                    }
    ...
    confirmPeerAction(action, data) {
        ...
        switch (action) {
            ...
            case 'mute':
                imageUrl = image.mute;
                title = 'Mute Audio  ' + whoMuteHideStop;
                text = 'Once muted, they can unmute temselves at any time.';
            case 'unmute':
                imageUrl = image.unmute;
                title = 'UnMute Audio  ' + whoMuteHideStop;
                text = 'Once unmuted, whey can mute temselves at any time.';
                break;
            case 'hide':
                imageUrl = image.hide;
                title = 'Hide Video  ' + whoMuteHideStop;
                text = 'Once hided, whey can unhide temselves at any time.';
            case 'unhide':
                imageUrl = image.unhide;
                title = 'Enable WebCam  ' + whoMuteHideStop;
                text = 'Once enabled webcam, whey can hide temselves at any time.';
            case 'stop':
                switch (action) {
                    case 'mute':
                        imageUrl = image.mute;
        ...
        Swal.fire({
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    ...
                    if (!data.broadcast) {
                        switch (action) {
                            case 'mute':
                                var peerAudioButton = this.getId(data.peer_id + '___pAudio');
                                if (peerAudioButton) peerAudioButton.innerHTML = _PEER.audioOff;
                                break;
                            case 'hide':
                                var peerVideoButton = this.getId(data.peer_id + '___pVideo');
                                if (peerVideoButton) peerVideoButton.innerHTML = _PEER.videoOff;
                            case 'unmute':
                                if (peerAudioButton) peerAudioButton.innerHTML = _PEER.audioOn;
                            case 'unhide':
                                if (peerVideoButton) peerVideoButton.innerHTML = _PEER.videoOn;
                            case 'stop':
            ...
            .then(() => {
                if (muteHideStopConfirmed) this.peerActionProgress(action, 'In progress, wait...', 2000, 'refresh');
            });
    }
