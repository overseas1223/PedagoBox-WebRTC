...
Create a meeting with a `HTTP request` containing the `API_KEY` sent to Pedagobox’s server. The response contains a `meeting` URL that can be `embedded` in your client within an `iframe`.
...
api: {
    // app/api
    keySecret: 'pedagobox_default_secret',
}
```
...
<iframe
    ...
    src="https://pe.pedagobox.com/join/your_room_name"
...
<iframe
    allow="camera; microphone; display-capture; fullscreen; clipboard-read; clipboard-write; autoplay"
    src="https://pe.pedagobox.com/newroom"
    style="height: 100vh; width: 100vw; border: 0px;"
></iframe>
```
